...
{
    public interface ICreatePersonValidator : IValidationService<Person>
    {

    }
    public class CreatePersonValidatior : FluentValidationService<Person>, ICreatePersonValidator
    {
        public CreatePersonValidatior()
        {
            RuleFor(x => x.FirstName)
                .NotEmpty()
                .WithMessage("Имя не должно быть пустым")
                .WithErrorCode("M01.01");

            RuleFor(x => x.LastName)
                .NotEmpty()
                .WithMessage("Фамилия не должна быть пустая")
                .WithErrorCode("M02.01");
